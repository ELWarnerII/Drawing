...
public class ImageComp implements ActionListener {
  ...
  /** Image showing the differences between the images in red. */
  Icon both;
  
  /** Label, for showing the various image. */
  JLabel display;

  private void usage() {
    System.err.printf( "usage: ImageComp <image1.pgm> [image2.pgm]\n" );
    System.exit( 1 );
  }
  ...
  private BufferedImage loadImage( String filename ) {
    try {
      ...
      if ( ! token.equals( "P2" ) ) {
        System.err.printf( "File %s doesn't look like a PGM file.\n", filename );
        usage();
      }
      ...
      if ( cmax != 255 ) {
        System.err.printf( "File %s doesn't look like a PGM image in the right format.\n", filename );
        usage();
      }

      // Make an image and populate it with grayscale values from the input file.
      BufferedImage img = new BufferedImage( width * 2, height * 2, BufferedImage.TYPE_INT_ARGB );
      for ( int i = 0; i < height; i++ )
        for ( int j = 0; j < width; j++ ) {
          int c = input.nextInt();
          img.setRGB( j * 2, i * 2, new Color( c, c, c ).getRGB() );
          img.setRGB( j * 2, i * 2 + 1, new Color( c, c, c ).getRGB() );
          img.setRGB( j * 2 + 1, i * 2, new Color( c, c, c ).getRGB() );
          img.setRGB( j * 2 + 1, i * 2 + 1, new Color( c, c, c ).getRGB() );
        }
    ...
    if ( args.length > 1 ) {
      ...
      for ( int i = 0; i < leftImg.getHeight(); i++ )
        for ( int j = 0; j < leftImg.getWidth(); j++ ) {
          ...
          if ( a != b )
            bothImg.setRGB( j, i, Color.RED.getRGB() );
          else
